...
myKeys conf@(XConfig {XMonad.modMask = modm}) = M.fromList $
  ...
  , ((modm,               xK_Return), windows W.swapMaster)
    -- Swap the focused window with the next windowe
...
main = do
  ...
  xmonad $ defaultConfig {
    ...
    , ((mod4Mask,  xK_b),  runOrRaiseAndDo "google-chrome" (className =? "Google-chrome") (\w -> focus w >> windows W.swapMaster))
    , ((mod4Mask,  xK_f),  runOrRaiseAndDo "urxvt -e screen" (title =? "amas") (\w -> focus w >> windows W.swapMaster))
    , ((mod4Mask,  xK_e),  runOrRaiseAndDo "eclipse" (resource =? "Eclipse") (\w -> focus w >> windows W.swapMaster))
    , ((mod4Mask,  xK_w),  runOrRaiseAndDo "QQ.exe" (className =? "Wine") (\w -> focus w >> windows W.swapMaster))
    , ((mod4Mask,  xK_n),  runOrRaiseAndDo "nautilus"  (className =? "Nautilus") (\w -> focus w >> windows W.swapMaster))
    , ((mod4Mask,  xK_g),  runOrRaiseAndDo "gvim"  (className =? "Gvim") (\w -> focus w >> windows W.swapMaster))
    , ((mod4Mask,  xK_i),  runOrRaiseAndDo "inkscape" (className =? "Inkscape") (\w -> focus w >> windows W.swapMaster))
